...
int main() {
    ...
    if (sockfd < 0) {
        std::cerr << "Client 1: Error creating socket" << std::endl;
        return -1;
    }
    ...
    if (bind(sockfd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        std::cerr << "Client 1: Bind failed" << std::endl;
        close(sockfd);
        return -1;
    }
    ...
    if (setsockopt(sockfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
        std::cerr << "Client 1: Failed to join multicast group" << std::endl;
        close(sockfd);
        return -1;
    }

    std::cout << "Client 1: Joined multicast group: " << MULTICAST_GROUP << std::endl;

    // Step 5: Receive messages from the multicast group
    while (true) {
        int len = recvfrom(sockfd, buffer, BUFFER_SIZE, 0, nullptr, nullptr);
        if (len < 0) {
            std::cerr << "Client 1: Receive failed" << std::endl;
            break;
        }
        buffer[len] = '\0';
        std::cout << "Client 1 received message: " << buffer << std::endl;
    }
